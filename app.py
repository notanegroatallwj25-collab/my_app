...
from flask import Flask, flash, redirect, render_template_string, request, url_for, session, has_request_context
...
PRIORITIES = {
    ...
    "not_urgent_important": {"label": "غير عاجل ومهم", "short": "غير عاجل ومهم", "class_name": "priority-blue", "dot": "blue", "sort": 2},
    "urgent_not_important": {"label": "عاجل وغير مهم", "short": "عاجل وغير مهم", "class_name": "priority-amber", "dot": "amber", "sort": 3},
    "not_urgent_not_important": {"label": "غير عاجل وغير مهم", "short": "غير عاجل وغير مهم", "class_name": "priority-slate", "dot": "slate", "sort": 4},
}
...
def get_today_tasks() -> list[dict[str, Any]]:
    ...
    if user_id is None:
        with connect_db() as conn:
            return [dict(row) for row in conn.execute("SELECT * FROM tasks WHERE task_date=? ORDER BY task_time, id", (today_iso(),)).fetchall()]
...
def get_stats() -> dict[str, Any]:
    user_id = get_current_user_id()
    with connect_db() as conn:
        query_user = "WHERE user_id = ?" if user_id else ""
        params = (user_id,) if user_id else ()
        total = conn.execute(f"SELECT COUNT(*) FROM tasks {query_user}", params).fetchone()[0]
        done = conn.execute(f"SELECT COUNT(*) FROM tasks {query_user} AND status='done'", params).fetchone()[0]
...
{% endwith %}
 ...
 <div><label style="display:block;font-size:13px;font-weight:700;color:#475569;margin-bottom:4px;">اسم المستخدم</label>
 <input class="field" type="text" name="username" placeholder="اكتب اسم المستخدم" required style="width:100%;padding:14px 16px;border-radius:14px;border:2px solid #e4e9f2;background:#fafcff;font-size:15px;transition:all 0.2s;outline:none;"></div>
 <div><label style="display:block;font-size:13px;font-weight:700;color:#475569;margin-bottom:4px;">كلمة المرور</label>
 <input class="field" type="password" name="password" placeholder="••••••••" required style="width:100%;padding:14px 16px;border-radius:14px;border:2px solid #e4e9f2;background:#fafcff;font-size:15px;transition:all 0.2s;outline:none;"></div>
...
<main class="shell">
 ...
 <div class="hero-top">
 <div class="brand"><div class="brand-mark">✓</div><div><p class="eyebrow">إدارة يومك بوضوح</p><h1>جدول أيزنهاور</h1><div class="date">{{ today_label }}</div></div></div>
 <div class="metrics"><div class="metric"><small>نقاط اليوم</small><strong>{{ today_score }}</strong></div><div class="metric streak"><small>الستريك</small><strong>{{ streak }}</strong></div><a class="btn btn-primary" href="{{ url_for('stats') }}">الإحصائيات</a></div>
 </div>
...
<section class="surface form-card">
 ...
 <select class="field" name="priority" aria-label="الأولوية">{% for key, item in priorities.items() %}<option value="{{ key }}">{{ item.label }}</option>{% endfor %}</select>
 <select class="field" name="repeat" aria-label="التكرار">{% for key, label in repeat_labels.items() %}<option value="{{ key }}">{{ label }}</option>{% endfor %}</select>
...
<nav class="toolbar" aria-label="أدوات سريعة">
 ...
 <form class="action-form" method="post" action="{{ url_for('add_daily_tasks_route') }}"><button class="btn btn-warning" type="submit">إضافة المهام اليومية التلقائية</button></form>
 <form class="action-form" method="post" action="{{ url_for('archive_old') }}" onsubmit="return confirm('سيتم نقل المهام القديمة إلى سجل المنجزات. هل تريد المتابعة؟')"><button class="btn btn-light" type="submit">أرشفة المهام القديمة</button></form>
 <form class="action-form" method="post" action="{{ url_for('clean') }}" onsubmit="return confirm('سيتم حذف المهام القديمة فقط. هل تريد المتابعة؟')"><button class="btn btn-light" type="submit">تنظيف المهام القديمة</button></form>
 <form class="action-form" method="post" action="{{ url_for('clean_done') }}" onsubmit="return confirm('سيتم حذف المهام المنجزة نهائيًا. هل تريد المتابعة؟')"><button class="btn btn-danger" type="submit">حذف المهام المنجزة</button></form>
 <form class="action-form" method="post" action="{{ url_for('reset_streak') }}" onsubmit="return confirm('هل تريد إعادة الستريك إلى صفر؟')"><button class="btn btn-light" type="submit">إعادة الستريك</button></form>
</nav>
...
{% for task in group %}
 ...
 <div class="task-main"><div><span class="task-time">{{ task.task_time }}</span><span class="task-desc">{{ task.description }}</span>{% if task.repeat != 'none' %}<span class="task-repeat">• {{ repeat_labels[task.repeat] }}</span>{% endif %}</div><div class="task-status">{% if task.status == 'done' %}✓{% elif task.status == 'late' %}⏱{% elif task.status == 'skipped' %}↷{% else %}○{% endif %}</div></div>
...
<script>
function toggleFocus(button){const columns=document.getElementById('task-columns');const enabled=columns.classList.toggle('focus-mode');[...columns.children].forEach((el,i)=>el.style.display=enabled && i>0 ? 'none':'block');button.textContent=enabled?'إظهار كل التصنيفات':'وضع التركيز';}
</script>
"""
...
def inject_context() -> dict[str, Any]:
    return {"priorities": PRIORITIES, "repeat_labels": REPEAT_LABELS, "status_labels": STATUS_LABELS}
...
def add_daily_tasks_route():
    ...
    schedule_pending_tasks()
    flash(f"تمت إضافة {added} مهمة يومية جديدة" if added else "المهام اليومية موجودة مسبقًا", "success")
    return redirect(url_for("index"))
